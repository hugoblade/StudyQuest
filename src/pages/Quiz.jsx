...
const sampleQuestions = [
  {
    ...
    options: ['A library for UI', 'B  A framework', 'C  A language', 'D  A database'],
    correct: 'A library for UI'
  },
  {
    question: 'What is JSX?',
    options: ['A JavaScript XML', 'B Java Extension', 'C JSON Syntax', 'D JSX is not real'],
...
const Quiz = () => {
  ...
  const handleAnswer = (isCorrect) => {
    ...
    if (current + 1 < questions.length) {
      ...
      // ✅ SAFETY CHECK: Only add XP if user is logged in
      if (currentUser) {
        const earnedXP = newScore * 10;
        addXP(currentUser.uid, earnedXP)
          .catch((err) => {
            console.error("Failed to save XP. Please log out and back in.", err);
            // Optionally show a user-friendly message
            setError("Session expired. Please logout and login again.");
          });
      } else {
        console.warn("User not logged in, skipping XP update.");
      }
      setFinished(true);
    }
  };
